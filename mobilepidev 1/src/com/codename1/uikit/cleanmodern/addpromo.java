...
import com.codename1.uikit.cleanmodern.entities.promo;
      ...
      /*  tb.setTitleCentered(false);
       ...
       Toolbar tb = new Toolbar(true);
        ...
        add(new Label("Today", "TodayTitle"));
                    ...
                    TextField pource = new TextField("", "pourcent", 20, TextField.NUMERIC) ;
                     TextField date = new TextField("", "date", 20, TextField.TEXT_CURSOR) ;
        ...
        FontImage arrowDown = FontImage.createMaterial(FontImage.MATERIAL_KEYBOARD_ARROW_DOWN, "Label", 3);
          btnValider.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                if ((pource.getText().length()==0)||(date.getText().length()==0))
                    Dialog.show("Alert", "tous les champs sont obligatoire", new Command("OK"));
                else
                { 
                    try {
                        promo ch = new promo((int)Float.parseFloat(pource.getText().toString()),date.getText().toString());
                        if( Promoservice.getInstance().addhotel(ch))
                        {
